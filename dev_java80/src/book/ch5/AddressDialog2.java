...
public class AddressDialog2 extends JDialog implements ActionListener {
	...
	//set메소드를 통해서 넘어온 4번째 값에 따라서 화면을 처리하는 콤퍼넌트 클래스의 수정모드에 대한 설정을 바꾸어 줍니다.
	//만일 true이면 setEnabled(true)로 설정하고
	//만일 false이면 setEnabled(false)로 설정하여 수정할수 없도록 할 계획
